...
const devices passwords[]{
	{0, "IMAGINE-CUP-IONET-bae3", "CDE34RFV"},
	{1, "IMAGINE-CUP-IONET-ed00", "ZAQ12WSX"},
	{2, "IMAGINE-CUP-IONET-BA80", "BGT56YHN"}};
...
void EmergencyConnectLED()
{
	SetLED(color[2][0], color[2][1], color[2][2]);
}
...
void Connect(const char *name, bool apMode = false)
{
	for (uint8_t i = 0; i < sizeof(passwords) / sizeof(devices); ++i)
	{
		if (strcmp(passwords[i].name, name) == 0)
		{
			if (apMode == true)
			{
				WiFi.beginAP((char *)passwords[i].name, passwords[i].pass);
				return;
			}
			else
			{
				wifi_status = WiFi.begin((char *)passwords[i].name, passwords[i].pass);
				return;
			}
		}
	}
...
bool EmergencyConnection()
{
	wifi_status = WL_DISCONNECTED;
	if (wifi_status != WL_CONNECTED)
	{
		Screen.print(1, "Emergency net");
		int numSsid = WiFi.scanNetworks();
		if (numSsid == -1)
		{
			Screen.print(2, "No networks");
			while (true)
				;
		}
...
void SetupDeviceId()
{
	byte mac[6];
	WiFi.macAddress(mac);
	sprintf(ap_name, "IMAGINE-CUP-IONET-%02x%02x", mac[4], mac[5]);
}

void SetupAPServer()
{
	Connect(ap_name, true);
}
...
void MessageToAzure(int messageId, char *payload, size_t len, char *timestamp)
{
